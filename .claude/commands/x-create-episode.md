...
### 6. 台本のリファイン（チーム並列レビュー × 3ラウンド）

生成した台本を「初見のリスナーがスムーズに理解できるか」の観点で3ラウンドかけてリファインする。
各ラウンドでAgent Teamsを使い、3人の専門レビュワーを**並列に**走らせる。

**レビュワーの役割:**
- **clarity-checker**: 専門用語や前提知識の説明が十分か。初見で「？」となる箇所がないか
- **flow-checker**: 話の流れが自然か。唐突な話題転換や論理の飛躍がないか
- **engagement-checker**: 聞いていて退屈にならないか。具体例・比喩・問いかけが効果的に使われているか

**各ラウンドの手順:**
1. 3人のレビュワーをAgentで並列に起動し、現在の台本を渡してレビューさせる
2. 各レビュワーから具体的な改善点（箇所の引用 + 改善案）を受け取る
3. 指摘を統合し、台本を改善する
4. 改善した台本で次のラウンドへ進む

**3ラウンド完了後:**
- 最終版の台本と、各ラウンドで行った改善のサマリー（箇条書き）をユーザーに提示する
...
台本の保存完了後、ユーザーに台本を確認してもらう。OKが出たら以下のステップ9〜11を順に実行する。
